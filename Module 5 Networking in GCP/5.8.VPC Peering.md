# VPC Peering (GCP)

In **Google Cloud Platform (GCP)**, **VPC Peering** allows two separate VPC networks to **communicate privately using internal IP addresses**.

👉 No public IP
👉 No VPN
👉 No internet involved

Traffic flows through Google’s private backbone.

---

# 🏢 Real-World Example

### Scenario: Shared Services Architecture

A company has:

* VPC-1 → Application VPC
* VPC-2 → Database VPC

Requirement:

App servers in VPC-1 must connect to DB servers in VPC-2 privately.

Instead of:

* Exposing DB publicly ❌
* Creating VPN ❌

They use **VPC Peering**.

Now:
App VM (10.0.1.2) → DB VM (10.0.2.2)
Direct private communication.

---

# 🔍 How VPC Peering Works

1. Two VPCs establish a peering connection.
2. Routes are automatically created.
3. Internal IP traffic is exchanged.
4. No NAT, no gateway required.
5. Firewall rules still apply.

Important:

* IP ranges must NOT overlap.
* Peering is non-transitive.

---

# 🧪 Step-by-Step Lab (From Scratch)

---

# Step 1️⃣ Create First VPC

Go to:

VPC Network → Create VPC

Configuration:

* Name: `vpc-app`
* Subnet mode: Custom
* Subnet:

  * Name: `app-subnet`
  * Region: us-central1
  * IP range: 10.0.1.0/24

Create.

---

# Step 2️⃣ Create Second VPC

Create another VPC:

* Name: `vpc-db`
* Subnet:

  * Name: `db-subnet`
  * Region: us-central1
  * IP range: 10.0.2.0/24

Create.

⚠ Ensure no overlapping IP ranges.

---

# Step 3️⃣ Create VM in Each VPC

Create:

VM-1:

* Name: `app-vm`
* Network: vpc-app
* Subnet: app-subnet

VM-2:

* Name: `db-vm`
* Network: vpc-db
* Subnet: db-subnet

---

# Step 4️⃣ Create VPC Peering Connection

Go to:

VPC Network → VPC Peering → Create Peering

From vpc-app:

* Peering name: `app-to-db`
* Peer network: vpc-db
* Enable route exchange

Create.

Now repeat from vpc-db:

* Peering name: `db-to-app`
* Peer network: vpc-app

Create.

Peering becomes ACTIVE.

---

# Step 5️⃣ Configure Firewall Rules

In both VPCs:

Create ingress rule:

* Allow ICMP
* Source: 10.0.0.0/8 (or specific peer subnet)

---

# Step 6️⃣ Test Connectivity

SSH into app-vm:

```id="w3z0ot"
ping 10.0.2.X
```

You should successfully reach db-vm internal IP.

No external IP needed.

---

# 🔄 Traffic Flow Explanation

Before Peering:

app-vm → db-vm ❌ Not reachable

After Peering:

app-vm (10.0.1.2)
↓
Google backbone
↓
db-vm (10.0.2.2)

No internet involved.

---

# 🧠 Important VPC Peering Rules

✔ No overlapping CIDR ranges
✔ Non-transitive
✔ No centralized routing
✔ Works across regions
✔ Firewall rules still control access

---

# 🚫 Non-Transitive Example

If:

VPC-A ↔ VPC-B
VPC-B ↔ VPC-C

VPC-A ❌ cannot reach VPC-C

Must create separate peering.

---

# 🏗 Enterprise Use Case

Large organization:

* Dev VPC
* Prod VPC
* Shared Services VPC

All peer with Shared Services VPC.

Used for:

* Central logging
* Central monitoring
* Shared database access

---

# 📌 Interview-Ready Explanation

VPC Peering allows private communication between two VPC networks using internal IP addresses without using VPN or internet. It requires non-overlapping IP ranges and is non-transitive.

---

# ✅ In Simple Words

VPC Peering:

* Connects two VPCs privately
* Uses internal IPs
* No internet required
* Must not overlap CIDR



